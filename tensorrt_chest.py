# File version run on C4140-16GB and copied to R740-P40 on Friday 10/05/2018
...
# Add python path to official models
sys.path.insert(0,'/workspace/tensorflow_models/')

from official.resnet import imagenet_preprocessing  # pylint: disable=g-bad-import-order

#_GPU_MEM_FRACTION = 0.50## threw out of memory errors with batch size 32, so memory fraction was increased as below
_GPU_MEM_FRACTION = 0.75## 
_WARMUP_NUM_LOOPS = 5
_LOG_FILE = "log_chest.txt"
_LABELS_FILE = "labellist_chest_x_ray.json"
...
def convert_savedmodel_to_frozen_graph(savedmodel_dir, output_dir):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    frozen_graph_def = tf.graph_util.convert_variables_to_constants(
        ...
        sess, graph.as_graph_def(), ["my_sigmoid_tensor"]) ##
...
def get_frozen_graph(graph_file):
  ...
  print("number of nodes before removing the unnecessary ones {}:".format(len(graph_def.node))) ##
  
  graph_def_cleaned = tf.graph_util.remove_training_nodes(graph_def)
  print ("number of nodes after removing the unnecesary ones {}:".format(len(graph_def_cleaned.node)))
         
  #return graph_def # wit throws this error: "ValueError: Node 'bn_conv1/cond/FusedBatchNorm/Switch_1' expects to be colocated with unknown node 'bn_conv1/gamma'"
...
def run_trt_graph_for_mode(
  ...
  print("g_name {}:".format(g_name), mode) ##
  return result
...
def top_predictions(result, n):
  ...
  probabilities = result[0] # select the element at the 0 index
  # Get the ids of most probable labels. Reverse order to get greatest first with [::-1] 
  ids = np.argsort(probabilities)[::-1] #Returns the indices that would sort an array
  return ids[:n]
...
def print_predictions(results, ids_are_one_indexed=False, preds_to_print=5):
  ...
  for mode, result in results:
    ...
    print("Precision: ", mode, pred_labels, pred_ids, result) ## 
...
def main(argv):
  ...
  if flags.native:
    ...
    print ("Second check") ##
    print (g_name) ##
    result = time_and_log_graph( # native precision thowing this error:ValueError: Negative dimension size caused by subtracting 8 from 7 for 'import/resnet_model/average_pooling2d/AvgPool' (op: 'AvgPool') with input shapes: [128,7,7,2048]  
...
class TensorRTParser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument(
        "--output_node", "-on", default="my_sigmoid_tensor", #"--output_node", "-on", default="softmax_tensor",
    ...
    self.add_argument(
        "--batch_size", "-bs", type=int, default=224, 
